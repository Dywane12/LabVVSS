...
public class Service {
    ...
    public int saveStudent(String id, String nume, int grupa) {
        ...
        if (id == null || id.trim().isEmpty() || !id.matches("[a-zA-Z0-9]+")) {
            return 0; // Invalid ID (null, empty, or contains special characters)
        }

        // Validate Name
        if (nume == null || nume.trim().isEmpty() || !nume.matches("^[a-zA-Z ]+$")) {
            return 0; // Invalid Name (null, empty, or contains numbers/special chars)
        }
        ...
        return 0; // Failure (student already exists)
    }
